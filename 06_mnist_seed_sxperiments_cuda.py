...
class MnistDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        image_values = torch.FloatTensor(self.data_df.iloc[index,1:].values).to(device) / 255.0
...
if __name__ == '__main__':
    ...
    for epoch in range(epochs):
        ...
        for label, image_data_tensor, target_tensor in mnist_dataset:
            ...
            D.train(G.forward(generate_random_seed(100)).detach(), torch.FloatTensor([0.0]).to(device))
            
            G.train(D, generate_random_seed(100), torch.FloatTensor([1.0]).to(device))
